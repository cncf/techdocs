...
**What this is.** A working specification for an AI-assisted, expert-supervised
...
- P-4: Reproducible and auditable. Every claim in a deliverable traces to a
  ...
  are versioned and re-runnable. Reproducible means convergent, not identical:
  two honest runs over the same material surface the same major findings, though
  details and wording differ (section 15).
...
- HC-5: Provenance required. Quantitative claims (counts, link status,
  ...
  source; no unverifiable figures in a deliverable. Citation is the working
  anti-hallucination control: a claim that names its source can be checked, and
  the verifier (section 4) depends on those handholds.
...
2. Accept. A technical writer triages and explicitly accepts the request, for
   every phase, not just A; eligibility alone does not start work (P-1). Triage
   can equally decline (section 12); acceptance is a choice, not a formality.
3. Draft. The drafter produces the deliverable as a draft PR. When the phase's
   ...
   Drafting may be decomposed by criteria section: successive delegations, each
   free to use a different model or effort (section 14), accumulate into the
   same draft PR. Decomposition changes how the draft is produced, not what is
   reviewed: the deliverable stays whole, and review (step 4) reads it as one
   document.
...
than sink time into editing. Each discarded draft is a decision point: restart,
hand-write, or stop; retries are chosen, never automatic. Repeated failure is
escalated to the administrator/platform owner, who may abort the run. An
assessment must never end up slower than doing it by hand.
...
- Phase C: Issue backlog. Each proposed issue is a separate file scoped to the
  ...
  coherence-checked by the reviewer; agent estimates are not reliable on their
  own.
...
by hand. The pilot, and any later evaluation of this process, measures output
(quality, time, and cost to produce), not documentation outcomes, which turn on
contributor effort beyond this process's control; we state the distinction so we
...
Filing also obligates no one: the program may decline the request, and the
project may withdraw at any point in the lifecycle (section 12). A request can
also be filed by the assessing team rather than the project, and HC-6's up-front
promise must not depend on who filed: phase A is not delegated until the project
contacts named in the request have been notified on the intake issue.
...
- Accept (step 2): the `/accept` command. A writer accepts by commenting
  ...
  labels it `triage/declined`, posts the reason when one is given, and closes
  the issue as not planned. The reason is optional: most declines should carry
  one, but a sensitive reason can go to the project privately instead of into
  the public record. Declining the intake means the assessment never starts.
  Declining a phase B or C tracking issue ends the assessment there, noted on
  the intake issue, which the workflow closes as not planned, so an open intake
  still means work in flight. A project's request to stop is honored the same
  way: the writer records a decline rather than leaving the request to sit
  unanswered.
...
- Build-time checks. Platform behavior we could not verify from public
  ...
  setup-step collection sharing the envelope, fits the session cap in a single
  delegation or leans on the per-section decomposition section 5 allows (section
  11); whether effort level can be pinned in a profile or anywhere else, the one
...
- Cost and usage reporting. Recording per-assessment expert hours and AI usage
  (credit or token draw) so G-1's compression claim can be judged against real
  numbers rather than impressions. Out of scope for the MVP.
- Template provenance. Propose adding the provenance block skeleton (section 15)
  to the methodology's templates as body content, under the same P-2 ordering as
  the section 6 issues-split proposal, and define what the template front
  matter's `author:` field carries on an AI-drafted deliverable. Until the
  methodology accepts it, the drafter's instructions carry the block format.
